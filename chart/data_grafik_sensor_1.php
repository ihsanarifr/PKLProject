<?php
  ...
  $resultSet = mysql_query("SELECT * FROM data_sensor_1 ORDER BY no DESC limit 30", $dbConn);
  $rowCount = mysql_num_rows($resultSet) or die(mysql_error());
 
  $xmlData = "<graph caption='Grafik Temperatur Suhu 1'
  yAxisMinValue='0' 
  yAxisMaxValue='100' animation='0' 
  canvasBorderColor='FFFFFF' xAxisName='Waktu' yAxisName='Celcius'
  ...
  for ($i=0;$i<$rowCount;$i++) {
  	...
  	$value1 = mysql_result($resultSet, $index, "celcius");
